...
// SOLUCIÓN MAESTRA: Nombre unificado 100% en minúsculas para evitar choques con el HTML
function iraPantalla(actualId, siguienteId) {
    const actual = document.getElementById(actualId);
    const siguiente = document.getElementById(siguienteId);

    if (actual && siguiente) {
        document.querySelectorAll('.step-section').forEach(seccion => {
            seccion.classList.remove('active');
        });

        siguiente.classList.add('active');

        if (siguienteId === 'step-4') {
            inicializarJuegoDos();
        }
        if (siguienteId === 'step-5') {
...
function inicializarJuegoUno() {
    ...
    bushImg.addEventListener('click', function manejarClic(e) {
        ...
        if (clicsArbusto === 4) {
            ...
            if (bushContainer) bushContainer.classList.add('hidden');
...
function configurarAdivinanzaNombre() {
    ...
    const alfabeto = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').slice(0, 25);
...
// =========================================================================
// 4. PASO 4: MINIJUEGO 2 - SOPA DE LETRAS 5X5 (ESTÁTICA POR LLAMADO SEGURO)
// =========================================================================
function inicializarJuegoDos() {
    ...
    // Resetea oyentes duplicados clonando los nodos de letras de forma estática
...
// =========================================================================
// 5. PASO 5: INVITACIÓN FINAL Y LOGÍSTICA DE ASISTENCIA WHATSAPP
// =========================================================================
function inicializarPantallaFinal() {
    ...
    const CONFIG_EVENTO = {
        ...
        lugarTexto: "Jardín Los Girasoles, Calle Selva #123" 
    };
